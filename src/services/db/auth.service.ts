...
class AuthService {
  ...
  public async forgetPassword(email: string) {
    ...
    await prisma.user.update({ where: { id: user.id }, data: { passwordResetCode: resetCode, passwordResetDate: expiresDate } })
    // 3) Send Email
    await emailSender.send('admin@gmail.com', user.email, 'Forgot Password', 'Your reset password token', `<h1>${resetCode}</h1>`)
  }
